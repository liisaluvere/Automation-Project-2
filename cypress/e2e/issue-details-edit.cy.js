describe("Issue details editing", () => {
  ...
  it("Should check values in issue priorities dropdown", () => {
    getIssueDetailsModal().within(() => {
      cy.get('[data-testid="select:priority"]')
        .invoke("text")
        .then((initialPriority) => {
          initialPriority = initialPriority.trim();
          priorityOptions.push(initialPriority);
        });
      cy.get('[data-testid="select:priority"]').click("bottomRight");
      cy.get('[data-testid^="select-option:"]').each(($option) => {
        const options = $option.text().trim();
        priorityOptions.push(options);
