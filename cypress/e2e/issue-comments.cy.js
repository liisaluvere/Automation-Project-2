describe("Issue comments creating, editing and deleting", () => {
  ...
  it("Should create, edit and delete a comment successfully", () => {
    const myComment = "TESTING_COMMENTING";
    const editedComment = "TESTING_COMMENTING_EDITED";
    const getAddCommentField = () =>
      cy.get('textarea[placeholder="Add a comment..."]');
    const getConfirmDeleteModal = () => cy.get('[data-testid="modal:confirm"]');
    const getIssueComment = () => cy.get('[data-testid="issue-comment"]');

    getIssueDetailsModal().within(() => {
      cy.contains("Add a comment...").click();

      getAddCommentField().type(myComment);
      ...
      getIssueComment().first().contains("Delete").click();
    });
